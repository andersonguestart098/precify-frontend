...
export default function InstitutionalPage() {
  return <Container maxWidth="lg" component="main" sx={{ py: { xs: 3, md: 6 } }}>
    ...
    <Grid container spacing={2}>
      {values.map(([title, description], index) => <Grid key={title} size={{ xs: 12, sm: 6 }}><Paper variant="outlined" sx={{ p: 3, height: "100%", borderRadius: 4, borderTop: `3px solid ${index % 2 ? "#54ad8f" : "#006b4f"}` }}><Typography variant="h6" fontWeight={850} mb={.75}>{title}</Typography><Typography color="text.secondary" lineHeight={1.6}>{description}</Typography></Paper></Grid>)}
    </Grid>
  </Container>;
}
