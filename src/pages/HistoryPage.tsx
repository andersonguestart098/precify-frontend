...
export default function HistoryPage() {
  ...
  return <Container maxWidth="md" sx={{ py: 4 }}><Typography component="h1" variant="h4" mb={2}>Histórico de buscas</Typography>{error && <Alert severity="error">{error}</Alert>}{busy ? <Typography role="status">Carregando…</Typography> : !rows.length && <Typography>Nenhuma pesquisa salva.</Typography>}<Stack gap={1} my={2}>{rows.map(row => <Button key={row.id} component={Link} to={`/produtos?q=${encodeURIComponent(row.query)}`} sx={{ justifyContent: "space-between", border: "1px solid #dce9e5", borderRadius: 2 }}><span>{row.query}</span><Typography variant="caption">{new Date(row.createdAt).toLocaleDateString("pt-BR")}</Typography></Button>)}</Stack><Button disabled={busy || !rows.length} onClick={() => void clear()}>Limpar histórico</Button></Container>;
}
