...
export function DesktopExploreNav() {
  ...
  return <Box component="nav" aria-label="Explore o Precify" sx={{
    ...
    bgcolor: "rgba(255,255,255,.9)", borderBottom: "1px solid rgba(25,138,74,.12)", backdropFilter: "blur(16px)",
    boxShadow: "0 9px 28px rgba(15,23,42,.045)",
  }}>
          ...
          return <Tooltip key={to} arrow placement="bottom" enterDelay={350} title={<Box sx={{ p: .4, maxWidth: 270 }}>
            ...
            <Stack component={RouterLink} to={to} aria-current={active ? "page" : undefined}
              alignItems="center" justifyContent="center" gap={.45} sx={{
                position: "relative", width: { md: 142, lg: 164 }, color: active ? "#0b6732" : "#5e7166",
                ...
                "&::before": {
                  ...
                  background: featured
                    ? "linear-gradient(135deg,rgba(25,138,74,.18),rgba(54,224,126,.1))"
                    : "linear-gradient(135deg,rgba(54,224,126,.13),rgba(25,138,74,.055))",
                  transition: "opacity 220ms ease, transform 220ms ease",
                },
                "&::after": active && !featured ? {
                  content: '""', position: "absolute", left: "25%", right: "25%", bottom: 0, height: 3,
                  borderRadius: "999px 999px 0 0", background: "linear-gradient(90deg,transparent,#36b96c,#0b6732,#36b96c,transparent)",
                } : undefined,
                "&:hover": { color: "#0b6732", transform: "translateY(-2px)", "&::before": { opacity: 1, transform: "scale(1)" } },
                "&:focus-visible": { outline: "2px solid #36e07e", outlineOffset: -4, borderRadius: 3 },
              ...
              <Box sx={featured ? {
                ...
                background: "linear-gradient(140deg,#0b6732,#198a4a 58%,#36e07e)", border: "3px solid #fff",
                boxShadow: "0 8px 22px rgba(25,138,74,.26)", position: "relative",
              } : { height: 32, display: "grid", placeItems: "center" }}>
                <Icon sx={{ fontSize: featured ? 27 : 26 }} />
                {featured ? <MicNoneRoundedIcon sx={{ position: "absolute", right: -8, bottom: 1, fontSize: 17, p: .25, boxSizing: "content-box", borderRadius: "50%", bgcolor: "#fff", color: "#0b6732", boxShadow: "0 2px 7px rgba(15,23,42,.15)" }} /> : null}
              </Box>
